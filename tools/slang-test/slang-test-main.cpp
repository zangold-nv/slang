...
// Requires SLANG_SPVDB_PATH to be set at CMake time (auto-detected from
// ../spvdb if present).
// ---------------------------------------------------------------------------
...
static std::string spvdb_value_str(const spvdb::Value& v)
{
    ...
    switch (v.kind)
    {
    case Kind::Bool:    return v.scalar.b ? "true" : "false";
    case Kind::Int32:   return std::to_string(v.scalar.i32);
    case Kind::UInt32:  return std::to_string(v.scalar.u32);
    case Kind::Int64:   return std::to_string(v.scalar.i64);
    case Kind::UInt64:  return std::to_string(v.scalar.u64);
    case Kind::Float32:
    {
...
static std::string spvdb_stop_reason_str(spvdb::StopReason r)
{
    ...
    switch (r)
    {
    case SR::Breakpoint:     return "Breakpoint";
    case SR::Step:           return "Step";
    case SR::EntryReached:   return "EntryReached";
    case SR::EntryFinished:  return "EntryFinished";
    case SR::Panic:          return "Panic";
    }
    return "Unknown";
}
    ...
    std::string* error_out)
{
    ...
    auto ensure_session = [&]() -> bool {
        if (!sess)
        {
            ...
            {
                *error_out =
                    std::string("spvdb: create_session failed: ") + r.error().message;
                return false;
            }
            sess = std::move(*r);
        }
    ...
    for (const auto& cmd_line : commands)
    {
        ...
        if (cmd == "entry")
        {
            if (tokens.size() < 2) { *error_out = "spvdb: entry requires <name>"; return ""; }
            entry_name = tokens[1];
            sess.reset();
        }
        ...
        else if (cmd == "backtrace")
        {
            ...
            {
                out << "frame[" << i << "]: " << bt[i].function_name
...
TestResult runSpvdbDebuggerTest(TestContext* context, TestInput& input)
{
    ...
    // libspvdb not linked — SLANG_SPVDB_PATH was not set at CMake time.
    auto reporter = context->getTestReporter();
    if (reporter)
        reporter->message(
            TestMessageType::Info,
            "SPVDB_DEBUGGER test skipped: spvdb not available (set SLANG_SPVDB_PATH)");
    return TestResult::Ignored;
#else
    auto outputStem = input.outputStem;
    ...
    // --- Step 1: compile the source file to SPIR-V with debug info ---
    {
        ...
        slangcCmdLine.setExecutableLocation(
            ExecutableLocation(context->options.binDir, "slangc"));
